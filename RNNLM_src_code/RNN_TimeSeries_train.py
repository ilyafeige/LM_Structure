...
def train_language_model(model, X_train, X_test, Y_batches, Y_test, 
                         num_epochs=1, print_frequency=0.1,
                         save_dir=None, save_file=None, N_loss_eval_points=50):
    ...
    num_data_seen = 0
    len_per_epoch = len(X_train) + 0.0
    model_type = model.model_type 

    Data_train = list(zip(X_train, Y_batches))
    ...
    for i in range(num_epochs):
        ...
        for x_batch, y_batch in Data_train:
            batch_loss = model.train_step(x_batch.T, y_batch)/(1.0*x_batch.size)
